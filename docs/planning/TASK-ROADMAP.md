...
| DONE    | Cursor/offset pagination standard | A shared offset metadata builder/contract standardizes API pagination, and mobile paginated RTK cache helpers now serialize by filters, merge later pages, de-duplicate stable ids, and cover matches, notifications, support, success stories, and wallet transaction summaries. Analytics-only endpoints can remain non-paginated unless product requirements change. |
...
| DONE    | Storybook component library       | React Native Storybook dependencies, Metro toggle, `.rnstorybook` config, cross-platform start scripts, provider decorators, and an initial auth social-button story are in place. Expand stories as new shared components stabilize.                                                                                                                |
| DONE    | Internationalization              | English/Hindi are launch-supported locales with namespace/key parity tests, static key CI checks, and a single supported-locale registry. Additional Indian languages are future content scope, not a launch blocker.                                                                                                                                 |
