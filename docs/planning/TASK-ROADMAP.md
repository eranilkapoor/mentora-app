...
| P0       | Finish store billing SDK/product mapping or keep purchase CTA guarded for release                              |    DONE | `expo-iap`, platform product mappings, guarded availability, checkout, and server verification are implemented; production console approval remains external. |
...
| PARTIAL | Auto-renewal/subscription lifecycle  | Receipt verification now preserves active/grace status, expiry, and cancelled auto-renew state; reconciliation updates existing subscriptions, expiry tasks and replay-safe webhooks are tested. Production Apple/Google notification credentials and sandbox evidence remain external. |
