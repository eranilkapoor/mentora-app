...
| Done   | Add shared `crm_module_records` MVP backend foundation for the partial CRM modules so each can create/list/update tenant-scoped operating records.                                                   |
| Done   | Extend shared `crm_module_records` MVP backend foundation to the remaining missing CRM modules: call center, WhatsApp CRM, scholarship, interview, events, field force, and other uncovered modules. |
...
| 3   | Organization Management | Partial        | `education-crm` has tenants and branches.                                                             | Add departments, teams, campuses, tenant branding, domains, channel settings, payment gateway settings, workflow settings.     |
| 4   | Lead Management         | Partial        | `education-crm` has leads, sources, stages, activities, assignments, public capture.                  | Add duplicate merge, tags, attachments, voice notes, capacity rules, transfer, advanced scoring, and lead import APIs.         |
...
| 6   | Application Management  | Partial        | `education-crm` has application create/list and application numbers.                                  | Add form builder, conditional fields, review stages, locks, history, document requirements, approvals.                         |
...
| 8   | Marketing Automation    | Partial        | `education-crm` has campaigns; admin UI has campaign surface.                                         | Add landing pages/forms, UTM inventory, conversion tags, A/B tests, drip workflows, ROI attribution.                           |
| 9   | Communication Module    | Partial        | `education-crm` has communications; notifications module exists.                                      | Add channel-specific delivery providers, scheduling, bulk sends, conversation inbox, call/video records.                       |
...
| 16  | Task Management         | Partial        | `education-crm` has CRM tasks create/list.                                                            | Add recurring tasks, SLA, escalation, reminders, task comments, task board APIs.                                               |
...
MVP update: all 30 CRM reference modules now share `crm_module_records` APIs for tenant-scoped create/list/update operations and `GET /api/v1/education-crm/module-coverage` for coverage discovery. They still need dedicated deep workflows listed above before they can be considered complete Meritto-level modules.
