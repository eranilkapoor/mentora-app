...
| 2   | User Management         | Product Ready  | `admin/rbac`, roles, permissions, users, user memberships, organization-user create/list, business-unit/campus/branch/department/team scope, CRM access review, refresh, export actions, and server-backed IAM hierarchy dropdowns are wired. | Remaining external only: email invite delivery provider and external HR/identity directory sync if selected.                   |
| 3   | Organization Management | Product Ready  | Organizations are backed by the `organizations` API/storage and support universities, colleges, institutes, schools, coaching brands, franchises, business units, campuses, branches, departments, teams, domains, branding, channel settings, lead sources/stages, dashboard scope, and CRM setup/export actions. | Remaining external only: live DNS/domain verification and payment gateway secret validation.                                   |
...
| Done   | Expanded User Management and Organization Management with organization users, departments, teams, campuses, branding, and channel settings APIs plus CRM action wiring.                                                                                                   |
