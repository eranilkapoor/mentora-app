...
| P0       | Finish store billing SDK/product mapping and provider authorization                                           | PARTIAL | `expo-iap`, product mappings, checkout, verification, and acknowledgement are implemented. Live diagnostics currently show Google Play API permission denial until the service account receives Purchases API access. |
...
| PARTIAL | Upgrade/downgrade/plan lifecycle     | Initial activation and billing screens exist; provider-driven replacement and renewal synchronization still require Google RTDN and Apple Server Notifications.                                               |
...
| PARTIAL | Auto-renewal/subscription lifecycle  | Initial receipt verification, acknowledgement, active/grace status, expiry, and replay reconciliation are implemented. Google RTDN, Apple Server Notifications V2, provider cancellation, and live renewal/refund evidence remain required. |
...
| PARTIAL | Automated regression tests        | Current audit passes 109 API unit suites/1,017 tests, 18 mobile suites/71 tests, and all 18 API E2E suites/45 tests, including Socket.IO and five DB-backed business journeys. MongoDB E2E binaries use a workspace-local cache that CI preserves. Coverage artifacts and targeted 100% thresholds exist; broader device, visual, and RTK cache integration suites remain. |
...
1. Grant the Google Play service account Purchases API permissions, rerun the live verification diagnostic, and complete a licensed-track purchase/restore test.
