...
| Partial | Add server-backed admin CRM state. The shell now loads tenants, module coverage, and available `module_records` rows through Redux async state and shows API fallback status; dedicated module APIs still need authenticated binding per module.         |
...
| Partial | Add tenant-aware auth context, guards, and repository helpers so every CRM query is scoped by `tenantId`. Shared tenant ObjectId helpers now protect MVP CRM services; next step is request-context enforcement after real CRM auth.                     |
| Partial | Build admin CRM lead list/detail, application pipeline, task board, campaign center, communication center, and reports. Frontend MVP now has consistent interactive lists, filters, actions, and detail drawers; next step is authenticated API binding. |
...
MVP update: all 30 CRM reference modules now share `module_records` APIs for tenant-scoped create/list/update operations and `GET /api/v1/module-records/coverage` for coverage discovery. The admin CRM frontend now uses Bootstrap, Font Awesome, and Redux Toolkit, and gives every module a usable enterprise MVP surface with navigation, filters, action buttons, listing rows, detail drawer, status indicators, server workspace status, and operational insight cards. The public website demo-request form now posts through a same-origin Next.js route and forwards to `/api/v1/leads/public` when the API base URL is configured. They still need dedicated deep workflows listed above before they can be considered complete Meritto-level modules.
