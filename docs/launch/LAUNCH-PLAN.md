...
| Task                              |      Status | Evidence / Notes                                                                                                                                                        |
| --------------------------------- | ----------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Privacy Policy screen/content     |        Done | Mobile feature exists and `mentora-public-website/privacy-policy.html` provides a public-page source. Needs hosted production URL for Play Console.                     |
...
| Task                                |      Status | Evidence / Notes                                                                                                                                                      |
| ----------------------------------- | ----------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Student/guardian safety controls    |        Done | Parent-student permissions, parental controls, schedule approval flags, AI tutor restrictions, and safety event logging exist in the learning domain.                 |
| AI tutor safety moderation          | Partly Done | Deterministic message moderation and safety event logging exist; provider-level moderation and human review operations remain before production child/student launch. |
| Photo/document privacy              | Partly Done | Profile media and student document section support exists. Needs QA across student, parent, support, and admin visibility.                                            |
...
| AI/manual media moderation          | Partly Done | Media moderation hooks/config exist; AI tutor safety events exist; provider credentials and admin review process need launch decision.                                |
...
| Learning entitlements          |        Done | `learning_entitlements` gate AI tutor access by student, subject, schedule, quantity, status, and expiry.                                                                                                                                   |
| Downgrade/expiry behavior      | Partly Done | Expiry task exists and AI tutor checks entitlements; final UX and feature-access regression testing needed.                                                                                                                                 |

## 6. Chat & Notifications

| Task                                      |      Status | Evidence / Notes                                                                                                                                             |
| ----------------------------------------- | ----------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| AI tutor chat API                         |        Done | Tutor sessions/messages, context endpoint, completion summaries, history, and mobile service hooks exist. Provider response integration remains placeholder. |
| Realtime chat via socket                  | Partly Done | Socket gateways and FE socket client usage exist. AI tutor currently uses HTTP service hooks; background/reconnect QA still needed.                          |
...
| Task                            |      Status | Evidence / Notes                                                                                                                     |
| ------------------------------- | ----------: | ------------------------------------------------------------------------------------------------------------------------------------ |
| Learning screen consistency     |        Done | Learn, Schedule, and Progress share the learning shell, headers, account switcher, API sync, and scroll behavior.                    |
...
| Public website hosting        |     Pending | `mentora-public-website` exists as static source; production hosting/domain/SSL and links in app-store forms still need setup.   |
...
| Learning reminder digest notifications           | Partly Done | Schedule reminder fields exist; final notification campaign rules and delivery QA need validation.                                    |
| Assessments and parent progress dashboard        |        Done | Question banks, questions, assessments, attempts, answers, results, topic progress, recommendations, and parent dashboard APIs exist. |
| Student activity and saved learning analytics    | Partly Done | Learning progress exists; full timeline analytics and business dashboard exports need verification.                                   |
...
9. Done - Add public web pages for account deletion, privacy, terms, community guidelines, plans, and support under `mentora-public-website`.
...
12. Done - Verify key learning screens use consistent headers/scroll behavior and API-driven states.
13. Pending - Host `mentora-public-website` and update Play Console/App Store/legal links to production URLs.
14. Pending - Replace AI tutor placeholder response with selected model provider, provider moderation, and usage metering.
