...
| `MATCH_DAILY_DIGEST_*`                                                  | Daily match digest enablement, dry-run, and batch limits    |
...
| `EXPO_PUBLIC_STORE_BILLING_ENABLED`        | Enables native Apple/Google subscription checkout once store products are configured |
...
Shared TypeScript API shapes live in:

```text
packages/api-contract/src/index.ts
```

The mobile app maps `@matchmate/api-contract` through `match-mate-mobile-app/tsconfig.json` and consumes those shared types from RTK Query services, currently focused on membership, billing, payment orders, store subscription verification, coupons, invoices, and subscription lifecycle requests.
...
Swagger remains available in non-production for endpoint/DTO inspection. The shared contract package reduces drift for high-change client-facing payment and subscription surfaces; a generated OpenAPI SDK is still a future upgrade path.
...
Socket.IO supports real-time chat, read receipts, delivery status, typing indicators, reactions, presence, block/report effects, and in-app notification events. The client connects with the same auth/session context used by REST calls.
