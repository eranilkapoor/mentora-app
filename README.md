...
For **10K monthly users**, don’t start with heavy ECS/Kubernetes. Start simple, scalable, and cost-controlled.
...
| Domain/SSL     | **Route 53 + ACM**                            | `api.yourdomain.com`                      |
...
I would **not** recommend Amazon DocumentDB at your current scale. It is more expensive and not fully identical to MongoDB behavior. Even AWS’s own DocumentDB pricing examples show instance-based costs that can become high quickly. ([Amazon Web Services, Inc.][4]) MongoDB Atlas is usually simpler for a NestJS + Mongoose app.
...
[1]: https://docs.aws.amazon.com/elasticbeanstalk/latest/dg/Welcome.html?utm_source=chatgpt.com "AWS Elastic Beanstalk"
[2]: https://aws.amazon.com/ses/pricing/?utm_source=chatgpt.com "Amazon SES pricing"
[3]: https://aws.amazon.com/end-user-messaging/pricing/?utm_source=chatgpt.com "AWS End User Messaging Pricing"
[4]: https://aws.amazon.com/documentdb/pricing/?utm_source=chatgpt.com "Amazon DocumentDB Pricing"
[5]: https://www.techradar.com/pro/security/experts-warn-amazons-simple-email-service-is-being-abused-to-launch-massive-volume-of-phishing-attacks?utm_source=chatgpt.com "Experts claim Amazon's Simple Email Service is being abused to launch phishing campaigns"

Yes, for a new launch with low traffic, you can **minimize cost heavily**. You don’t need Redis, SES, load balancer, CloudFront, or multiple servers on day one.
...
Elastic Beanstalk has no extra platform charge, but you still pay for EC2/S3/etc. under it. For lowest cost, a single EC2 instance with Docker or PM2 is cheaper and simpler initially. ([Amazon Web Services, Inc.][1])
...
MongoDB Atlas has a free tier and low-cost shared/flex options, while dedicated clusters start higher. ([MongoDB][2])
...
For NestJS + sockets, I would prefer **at least 1 GB RAM**, better **2 GB RAM** if you also run image processing or heavy cron jobs.

---

# Should you skip Redis?

Yes, initially you can skip Redis.
...
online users
socketId mapping
temporary OTP attempts
simple rate limit cache
```

But keep in mind:

```txt
Local memory works only with 1 server.
If server restarts, socket state is lost.
If you scale to 2 servers, users connected to server A are not visible on server B.
```
...
Redis online user store
Redis rate limiting
Redis queues
```

## Production-safe local socket note
...
This is fine until you need horizontal scaling.
...
# S3 vs local folder for profile images

## My recommendation: Use S3 even at launch.

S3 is usually very cheap for small apps. AWS S3 Standard storage is roughly priced per GB-month, and the first 50 TB tier is listed around $0.023/GB-month on AWS pricing pages. ([Amazon Web Services, Inc.][3])

For example:

```txt
10 GB images ≈ very low monthly storage cost
100 GB images ≈ still manageable
```

## Why S3 is better than local folder

Use S3 because:

```txt
EC2 local disk can be lost during redeploy/migration
Backups are your responsibility
Scaling becomes difficult
Images should not depend on one server
Mobile apps need stable public URLs
S3 integrates with CloudFront later
Cleaner security using signed URLs
```

## When local folder is acceptable

Only use local storage if:

```txt
private testing
very early MVP
no important user data
you take backups
you are okay losing files during server migration
```

If you still want local, store in:

```txt
/var/www/matchmate/uploads
```
...
because deployment may overwrite it.
...
But for matrimonial apps, profile photos are core data. I would still use S3 from day one.
...
This will keep your monthly cost very low while still being production-friendly enough for an early matrimonial app launch.

[1]: https://aws.amazon.com/elasticbeanstalk/pricing/?utm_source=chatgpt.com "AWS Elastic Beanstalk Pricing"
[2]: https://www.mongodb.com/products/platform/atlas-cloud-providers/aws/pricing?utm_source=chatgpt.com "AWS MongoDB Pricing"
[3]: https://aws.amazon.com/s3/pricing/?utm_source=chatgpt.com "S3 Pricing"

Your decided setup is good for low-cost launch:
...
You mainly still need:

```txt
1. EC2 server
2. Elastic IP
3. Security Group
4. S3 bucket + IAM user/role
5. MongoDB Atlas network access
6. Nginx reverse proxy with WebSocket support
7. PM2 production process
8. Cloudflare DNS A record
9. GitHub Actions deployment
10. SSL on EC2/Nginx
```

Even if your Hostinger site already has SSL, your API subdomain on EC2 still needs SSL separately unless Cloudflare is doing SSL termination. Best setup: use Cloudflare DNS + Nginx + Certbot on EC2.
...
```bash
sudo mkdir -p /var/www/matrimonial-api
sudo chown -R ubuntu:ubuntu /var/www/matrimonial-api
```
...
```bash
nano /var/www/matrimonial-api/.env
```
...
```bash
sudo nano /etc/nginx/sites-available/matrimonial-api
```
...
```bash
sudo ln -s /etc/nginx/sites-available/matrimonial-api /etc/nginx/sites-enabled/matrimonial-api
sudo nginx -t
sudo systemctl reload nginx
```
    ...
    steps:
      ...
      - name: Upload archive to EC2
        ...
        with:
          ...
          key: ${{ secrets.EC2_SSH_KEY }}
          source: api.tar.gz
          target: /var/www/matrimonial-api
...
✅ Mobile app API URL updated to https://api.yourdomain.com
```

This setup is enough to run your complete NestJS API server at low cost while still being production-friendly for your first launch.

[1]: https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-security-groups.html?utm_source=chatgpt.com "Amazon EC2 security groups for your EC2 instances"
[2]: https://pm2.keymetrics.io/?utm_source=chatgpt.com "PM2 - Home"
...
[4]: https://nginx.org/en/docs/http/websocket.html?utm_source=chatgpt.com "WebSocket proxying"
