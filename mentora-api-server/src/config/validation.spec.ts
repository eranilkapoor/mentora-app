...
const productionEnv = () => ({
  NODE_ENV: 'production',
  API_BASE_URL: 'https://api.mentora.example',
  APP_WEB_URL: 'https://mentora.example',
  ALLOWED_ORIGINS: 'https://mentora.example',
  DB_DRIVER: 'mongo',
  MONGO_URI: 'mongodb://localhost:27017/mentora',
  CACHE_DRIVER: 'redis',
  REDIS_HOST: 'redis.mentora.internal',
  STORAGE_DRIVER: 's3',
  AWS_REGION: 'ap-south-1',
  AWS_S3_BUCKET: 'mentora-production-private',
  AWS_S3_BASE_URL: 'https://media.mentora.example',
  ...
  JWT_AUDIENCE: 'mentora-user',
  JWT_REFRESH_AUDIENCE: 'mentora-user-refresh',
...
describe('production environment validation', () => {
    ...
    expect(
      ENV_VALIDATION_SCHEMA.validate({
        ...
        GOOGLE_PLAY_RTDN_AUDIENCE:
          'https://mentora.webnza.com/api/v1/payments/google-play/rtdn',
        GOOGLE_PLAY_RTDN_SERVICE_ACCOUNT_EMAIL:
          'mentora-rtdn-push@project.iam.gserviceaccount.com',
      }).error,
    ).toBeUndefined();
  });
  ...
  it('requires complete SMTP settings when SMTP email delivery is enabled', () => {
    const { error } = ENV_VALIDATION_SCHEMA.validate({
      ...
      NOTIFICATION_EMAIL_FROM: 'noreply@mentora.example',
    });
  ...
  it('accepts SMTP email delivery from DSN or discrete Hostinger settings', () => {
    expect(
      ENV_VALIDATION_SCHEMA.validate({
        ...
        NOTIFICATION_EMAIL_SMTP_DSN:
          'smtps://noreply%40mentora.example:secret@smtp.hostinger.com:465',
      }).error,
    ).toBeUndefined();
    ...
    expect(
      ENV_VALIDATION_SCHEMA.validate({
        ...
        NOTIFICATION_EMAIL_SMTP_USERNAME: 'noreply@mentora.example',
