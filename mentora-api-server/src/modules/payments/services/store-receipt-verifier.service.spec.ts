...
describe('StoreReceiptVerifierService', () => {
  ...
  it('verifies an active Google Play subscription', async () => {
    ...
    config.set('payments.googlePlay.packageName', 'com.mentora.app');
    config.set(
  ...
  it('acknowledges a verified Google Play subscription purchase', async () => {
    const { privateKey } = generateKeyPairSync('rsa', { modulusLength: 2048 });
    config.set('payments.googlePlay.packageName', 'com.webnza.mentora');
    config.set(
      'payments.googlePlay.serviceAccountJson',
      JSON.stringify({
        client_email: 'billing@project.iam.gserviceaccount.com',
    ...
    expect(global.fetch).toHaveBeenLastCalledWith(
      expect.stringContaining(
        '/purchases/subscriptions/mentora_gold/tokens/purchase-token:acknowledge',
      ),
  ...
  it('loads Google Play service-account credentials from a file path', async () => {
    const { privateKey } = generateKeyPairSync('rsa', { modulusLength: 2048 });
    const directory = mkdtempSync(join(tmpdir(), 'mentora-google-play-'));
  ...
  it('verifies an Apple App Store transaction in sandbox', async () => {
    ...
    config.set('payments.apple.bundleId', 'com.mentora.app');
    config.set(
