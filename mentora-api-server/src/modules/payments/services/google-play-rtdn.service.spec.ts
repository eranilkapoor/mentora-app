...
describe('GooglePlayRtdnService', () => {
  const config = new Map<string, unknown>([
    ...
    ['payments.googlePlay.packageName', 'com.webnza.mentora'],
  ]);
  const configService = {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(OAuth2Client.prototype, 'verifyIdToken').mockResolvedValue({
      getPayload: () => ({
        email_verified: true,
        email: 'mentora-rtdn-push@project.iam.gserviceaccount.com',
      }),
    } as never);
  ...
  it('accepts authenticated test notifications without reconciliation', async () => {
    await expect(
      service.process(
        'Bearer signed-token',
        notification({
          packageName: 'com.webnza.mentora',
