...
export type UniversityPartnerDocument = HydratedDocument<UniversityPartner>;

@Schema({ collection: COLLECTION_NAMES.UNIVERSITY_PARTNER, timestamps: true })
export class UniversityPartner {
  @Prop({
    type: Types.ObjectId,
...
export const UniversityPartnerSchema =
  SchemaFactory.createForClass(UniversityPartner);
UniversityPartnerSchema.index({ organizationId: 1, status: 1, dueAt: 1 });
