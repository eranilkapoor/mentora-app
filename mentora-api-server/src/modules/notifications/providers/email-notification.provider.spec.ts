  ...
  it('sends Hostinger-style SMTP email over implicit TLS', async () => {
    ...
    config.get.mockImplementation((key: string, fallback?: unknown) => {
      const values: Record<string, unknown> = {
        ...
        'notification.email.from': 'noreply@mentora.test',
        'notification.email.smtp.host': 'smtp.hostinger.com',
        'notification.email.smtp.port': 465,
        'notification.email.smtp.username': 'noreply@mentora.test',
    ...
    expect(socket.writes).toEqual(
      expect.arrayContaining([
        ...
        'AUTH LOGIN\r\n',
        `${Buffer.from('noreply@mentora.test').toString('base64')}\r\n`,
        `${Buffer.from('secret').toString('base64')}\r\n`,
        'MAIL FROM:<noreply@mentora.test>\r\n',
