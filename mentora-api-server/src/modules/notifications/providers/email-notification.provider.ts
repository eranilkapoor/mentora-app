...
export class EmailNotificationProvider implements NotificationChannelProvider {
    ...
    try {
      const input: SendEmailCommandInput = {
        ...
        Content: {
          Simple: {
            Subject: {
              Data: payload.subject ?? payload.title ?? 'Mentora Notification',
            },
            Body: {
              Text: {
  ...
  private buildSmtpMessage(
    ...
    const subject = this.sanitizeHeader(
      payload.subject ?? payload.title ?? 'Mentora Notification',
    );
    ...
    return [
      ...
      `Subject: ${subject}`,
      'MIME-Version: 1.0',
      'Content-Type: multipart/alternative; boundary="mentora-notification"',
      '',
      '--mentora-notification',
