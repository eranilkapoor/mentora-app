...
import { ActiveTab, FormErrors, SocialProvider } from '../Login.types';
...
export function useLoginForm() {
  ...
  // ─── Dispatch credentials helper ─────────────────────────────────────────

  const applyCredentials = useCallback(
    async (data: { accessToken: string; refreshToken?: string; user: unknown }) => {
      dispatch(
        setCredentials({
          accessToken: data.accessToken, // aligns with authSlice field name
  ...
  const handleGetOtp = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await sendOtp({ country_code: countryCode, phone }).unwrap();
      if (!response.success) {
